...
function buildEvaluationPrompt(lesson, templateType) {
  ...
  const sectionsTemplate = RUBRIC_CRITERIA.map((c) =>
    [
      ...
      '    "feedback": "1-2 sentence explanation referencing the lesson content"',
      "  }",
    ].join("\n")
  ).join(",\n");
  ...
  return [
    ...
    "- Use only the ratings: high, medium, or low.",
    ...
    "- Each section must have: id (string), title (string), rating (high|medium|low), feedback (string).",
    "- rating must be exactly one of: high, medium, low — no other values.",
